...
#' @param subgroup Logical.Logical. If TRUE, subgroups are generated based on similarities in model features using the \code{walktrap.community} function from the \code{igraph} package.
#' @details
...
#                 col.names = FALSE)
    ...
    if (plot==TRUE){
      ...
      # if (agg==TRUE) plotind <- file.path(individual,"allPlot.pdf")
      pdf(plotind)
...
wrapup <- function(indsem.internal.out,
  ...
  if (subgroup == TRUE & all.diff.sub == FALSE){
    ...
    for (p in 1:n.subgroups){
      ...
      rownames(sub.paths.lag) <- lvarnames[(rois+1):(rois*2)]
      ...
      # sub.paths <- sub.paths[(rois+1):(rois*2),(1:rois*2)]
      # rownames(sub.paths) <- lvarnames[(rois+1):(rois*2)]
      # colnames(sub.paths) <- lvarnames[1:(rois*2)]

      if (sub.subjects != 1)  {
        #write.csv(sub.paths, file=file.path(subgroup.dir,paste("subgroup",p,PathCounts.csv",sep=""), row.names=TRUE)
  ...
  if (agg==FALSE){
    ...
    rownames(final.paths.lag) <- lvarnames[(rois+1):(rois*2)]
    ...
    # final.paths <- final.paths[(rois+1):(rois*2),(1:rois*2)]
    # rownames(final.paths) <- lvarnames[(rois+1):(rois*2)]
    # colnames(final.paths) <- lvarnames[1:(rois*2)]
    # write.csv(final.paths, file=file.path(out,"finalPathCounts.csv"), row.names=TRUE)
...
#       all.elements.summary  <- all.elements.summary[order(-all.elements.summary$count),]
  ...
  if (agg == FALSE){
    ...
    #write.csv(all.elements.summary,file.path(out,"allPathSummary.csv"),row.names=FALSE)
  }
}
################################################################################
################################################################################
